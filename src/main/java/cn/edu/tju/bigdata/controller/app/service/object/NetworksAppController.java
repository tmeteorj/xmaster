...
public class NetworksAppController extends BaseController {
    ...
    public String networks() {
        return Common.BACKGROUND_PATH + "/app/algorithmresourcesmanage/networksapplication";
    }
...
//        params.set("where", String.format("where deleted_mark = %d and held_time >= STR_TO_DATE('%s', '%%Y-%%m-%%d') and held_time < STR_TO_DATE('%s', '%%Y-%%m-%%d')", EmDeletedMark.VALID.getCode(), "2016-01-01", "2016-01-28"));
        ...
        */
        ...
        for (int i = 0; i < graphMatrix.getNodeCount(); i++) {
            ...
            node.put("community", communities.getCommunityIndex(i));
            nodes.add(node);
        }
        ...
        while (iterator.hasNext()) {
            ...
            link.put("sourceValue", graphMatrix.getNode(coord[0]));
            link.put("targetValue", graphMatrix.getNode(coord[1]));
            links.add(link);
        }
